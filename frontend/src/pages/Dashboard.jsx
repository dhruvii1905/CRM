    ...
    <div className="p-4 sm:p-6 lg:p-8 w-full">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 mb-6 sm:mb-8">
        {cards.map(c => (
          <button key={c.label} onClick={() => setActive(active === c.label ? null : c.label)}
            className={`text-left p-5 sm:p-6 rounded-2xl border-2 transition-all shadow-sm hover:shadow-md w-full
              ${active === c.label
            ...
            <p className="text-3xl sm:text-4xl font-bold">{data[c.label].length}</p>
            <p className="text-base sm:text-lg font-semibold mt-1">{c.label}</p>
      ...
      {active && (
        <div className="bg-white dark:bg-zinc-800 rounded-2xl shadow border border-gray-100 dark:border-zinc-700 overflow-hidden">
          <div className="px-4 sm:px-6 py-4 border-b border-gray-100 dark:border-zinc-700 flex items-center justify-between">
            <h2 className="text-base sm:text-lg font-semibold">{active}</h2>
            <div className="flex items-center gap-3">
              <span className="text-sm text-gray-400">{data[active].length} records</span>
              <button onClick={() => setActive(null)} className="text-gray-400 hover:text-black dark:hover:text-white text-lg transition">✕</button>
            </div>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-sm min-w-max">
              <thead className="bg-gray-50 dark:bg-zinc-900 text-left">
                <tr>
                  {cols[active].map(h => (
                    <th key={h} className="px-4 sm:px-5 py-3 font-semibold uppercase text-xs tracking-wider text-gray-500 dark:text-gray-400 whitespace-nowrap">
                      {h}
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                {data[active].length === 0 ? (
                  ...
                  <tr key={i} className="border-t border-gray-100 dark:border-zinc-700 hover:bg-gray-50 dark:hover:bg-zinc-700 transition">
                    {cols[active].map(col => (
                      <td key={col} className="px-4 sm:px-5 py-3 whitespace-nowrap">
